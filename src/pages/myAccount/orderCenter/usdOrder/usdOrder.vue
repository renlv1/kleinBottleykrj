        ...
        <div class="order-content">
          ...
          <table class="commonTableList" v-if="orderTab === 1">
            ...
            <tr>
              ...
              <td class="time">{{$t('usdMoney.text4')}}</td>
            ...
            <tr v-for="(item,index) in rechargeList" :key="index" class="cursor" @click="toRecharge(item)">
              <td>{{item.id}}</td>
              <!-- 币种为非BTC的要保留四位小数  BTC的要保留八位 -->
              <td class="time" v-show="item.depositCurrency !== 'BTC'">{{item.depositAmount.toFixed(4)}} {{item.depositCurrency === 'BTC' ? 'BTC' : 'USD'}}</td>
              <td class="time" v-show="item.depositCurrency === 'BTC'">{{item.depositAmount.toFixed(8)}} {{item.depositCurrency === 'BTC' ? 'BTC' : 'USD'}}</td>
          ...
          <table class="commonTableList" v-if="orderTab === 2">
            ...
            <tr>
              <td>{{$t('usdOrder.text5')}}</td>
              <td class="time">{{$t('rechargeRecord.text7')}}</td>
              <td class="time">{{$t('usdOrder.text7')}}</td>
              <td>{{$t('usdOrder.text8')}}</td>
              <td>{{$t('usdOrder.text9')}}</td>
            </tr>
            ...
            <tr v-for="(item,index) in withdrawList" :key="index"   class="cursor" @click="toWithdraw(item)">
              <td>{{item.id}}</td>
              <td class="time" v-show="item.payCurrency !== 'BTC'"><span>{{item.drawAmount.toFixed(4)}}</span> {{item.payCurrency}}</td>
              <td class="time" v-show="item.payCurrency === 'BTC'"><span>{{item.drawAmount.toFixed(8)}}</span> {{item.payCurrency}}</td>
              <td class="time">{{$changeDate(item.createAt, '/')}}</td>
          ...
          <table class="commonTableList" v-if="orderTab === 3" >
            ...
            <tr>
              ...
              <td class="time">{{$t('sieOrderDetail.text3')}}</td>
              <td>{{$t('usdOrder.text8')}}</td>
              <td>{{$t('usdOrder.text9')}}</td>
            </tr>
            </thead>
            <tbody>
            <tr v-for="(item,index) in usdTradeList" :key="index"  class="cursor" @click="toDetail(item)">
              <td>{{item.id}}</td>
              <td class="time">{{item.cashAmount | fourNumber}} USD</td>
              <td class="time">{{tradeType(item.etype)}}</td>
              <td>{{tradeStatus(item.state)}}</td>
              <td class="handle">{{$t('usdOrder.text10')}}</td>
            </tr>
            </tbody>
            <div v-if="usdTradeList.length === 0" class="queryErr">{{$t('orderManage.text8')}}</div>
            <v-turnPage v-if="usdTradeList.length !== 0" :propsPage="usdTradeTotal" @usdTradeTrun="usdTradeTurnPage"></v-turnPage>
          </table>
          <!--转账列表-->
          <table class="commonTableList" v-if="orderTab === 4">
            <thead>
            <tr>
              <td>{{$t('usdOrder.text5')}}</td>
              <td class="time">{{$t('usdOrder.text6')}}</td>
            ...
            <tbody >
            <tr v-for="(item,index) in transferList" :key="index"   class="cursor" @click="toTransferDetail(item)">
              <td>{{item.id}}</td>
              <td class="time">{{item.totalValue | fourNumber}} USD</td>
              <td class="time">{{recordType(item.recordType)}}</td>
              <td>{{ordStatus(item.orderStatus, item.recordType)}}</td>
              <td class="handle">{{$t('usdOrder.text10')}}</td>
            </tr>
            </tbody>
    ...
    <div class="mobile">
      <div class="manageUSD">
        ...
        <div class="manage-cont">
          ...
          <div class="ma-r">
            <div class="record-con" @click="$router.push({path: '/account/rechargeRecord', query: {status: 1}})">
              <span class="text">{{$t('usdOrder.text13')}}</span>
              <div class="icon"><img src="../../../../assets/images/myAccount/myDon/row-link-red.svg" alt=""></div>
            </div>
            <div class="record-con" @click="$router.push({path: '/account/withdrawalRecord', query: {status: 1}})">
