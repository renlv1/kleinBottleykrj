  ...
  <div class="mobile">
    <div class="management-head">
      <div class="head">
        ...
        <div class="title">USD{{$t('sieOrderDetail.text17')}}</div>
        <div class="right"></div>
      </div>
    </div>
    <div class="mobile-content">
      ...
      <div class="mobile-box">
        <ul class="tradeCenter" v-if="orderTab === 1">
          <li v-for="(item,index) in usdTradeList" :key="index" @click="toDetail(item)">
            <div class="left-dc">
              <div class="number">{{$t('sieOrderDetail.text20')}}: {{item.id}}</div>
              <div class="buy-time">{{$changeDate(item.createAt, '/')}}</div>
            </div>
            <div class="right-dc">
              <div class="buy-money"><span v-if="item.etype === 1">{{$t('sieOrderDetail.text21')}}</span><span v-if="item.etype === 2">{{$t('sieOrderDetail.text22')}}</span>{{$t('sieOrderDetail.text23')}}: {{item.cashAmount | fourNumber}} USD</div>
              <div class="status">{{tradeStatus(item.state)}}</div>
            </div>
          </li>
          <div v-if="usdTradeList.length === 0" class="error">{{$t('orderManage.text8')}}</div>
          <v-turnPage v-if="usdTradeList.length !== 0" :propsPage="usdTradeTotal" @usdTradeTrun="usdTradeTurnPage" ref="turnPage"></v-turnPage>
        </ul>
...
export default {
  ...
  methods: {
    ...
    getPathSelf () {
      if (this.$route.path === '/account/usdTradeRecord') {
