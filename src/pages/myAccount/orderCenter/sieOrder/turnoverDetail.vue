      ...
      <div class="mobile">
        <div class="management-head">
          <div class="head">
            ...
            <div class="title"><span v-if="type === 1">SIE</span><span v-if="type === 2">USD</span>{{$t('invitedRecord.text7')}}</div>
            <div class="right"></div>
          </div>
        </div>
          ...
          <div class="mobile-content">
            ...
            <ul>
              <li v-for="(item,index) in orderDetailList" :key="index"   class="cursor" >
                <div class="left-dc">
                  <div class="number"> {{$t('invitedRecord.text8')}}：{{item.tradePrice | fourNumber}} USD</div>
                  <div class="time">{{$changeDate(item.createAt, '/')}}</div>
                </div>
                <div class="right-dc">
                  <div class="transfer-money">{{$t('invitedRecord.text9')}}：{{item.tradeAmount | fourNumber}} SIE</div>
                  <div class="status">{{$t('invitedRecord.text10')}}：<span style="color: #FC534C">{{item.tradeTotalPrice | fourNumber}} USD</span></div>
                </div>
              </li>
...
export default {
  ...
  methods: {
    ...
    getPathSelf () {
      if (this.$route.path === '/account/sieOrder/turnoverDetail') {
        if (window.innerWidth > 1200) {
          // console.log(this.$route.query.obj)
          if (this.type === 1) {
            this.$router.push({path: '/account/sieOrder/sieOrderDetail', query: {obj: this.obj1}})
          } else {
            this.$router.push({path: '/account/usdOrder/usdTradeDetail', query: {obj: this.obj1}})
          }
        }
      }
    },
