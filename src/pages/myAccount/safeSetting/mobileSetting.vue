<template>
    <div>
      <div id="fixed-top-header">
        ...
        <h3 class="fixed-title">{{$t('bindPhone.text16')}}</h3>
        <div class="padding">
      ...
      <div class="pc-content">
        <div class="property-title">
          <h6><router-link to="/account/safettingCenter" class="property">{{$t('bindPhone.text15')}} > </router-link><span class="transfer">{{$t('bindPhone.text16')}}</span></h6>
        </div>
        <div class="bindPhone secret-wrap">
          <div class="secret-wrap-ct">
            <div class="inputTipBox inputTipBox-select">
              <div class="inputTipDiv" ref="discountBtn">
                <input class="secret-input secret-input-bordernone select-input" type="text" readonly disabled :value="codeValue">
                <a class="discount-btn" @click="codeSelectShow = !codeSelectShow"><i class="arrowDown" :class="{arrowActive: codeSelectShow}"></i></a>
                ...
                <div @click="codeSelectShow = !codeSelectShow" class="input-mask"></div>
              </div>
            </div>
  ...
  methods: {
    ...
    sendPhoneCode () {
      if (this.phoneNum === '') {
        ...
        this.$http.post(this.$api + '/user/sendSmsCode', formData, {headers: {'secret-language': ajaxLang}}).then((res) => {
          if (res.data.success) {
    ...
    completeFun () { // 完成
      if (this.phoneNum === '') {
        ...
        this.$http.post(this.$api + '/user/modifyUserInfo', formData, {headers: {'secret-language': ajaxLang}}).then((res) => {
          if (res.data.success) {
