    ...
    <div class="pc">
      <div class="pc-content">
        <div class="property-title order-title">
          <h6>
            <router-link to="/account/exchanges" class="property">{{$t('exchanges.text1')}} ></router-link>
            <span class="transfer">{{$t('sieOrderDetail.text29')}}</span></h6>
        </div>
        <div class="order-progress">
          <h6>1.{{tradeType(obj.etype)}}{{$t('sieOrderDetail.text1')}}</h6>
          <p><span style="color: #FC534C"
                   v-if="obj.state === 1 || obj.state === 3">{{obj.tradeAmount .toFixed(8)}}</span> <span style="color: #FC534C"
            v-if="obj.state !== 1 && obj.state !== 3">{{obj.tradeAmount .toFixed(8)}}</span> <span style="color: #FC534C"
            v-if="obj.etype === 1">{{obj.buyCurrency}}</span><span v-if="obj.etype === 2">{{obj.sellCurrency}}</span> /
            <i>{{obj.amount .toFixed(8)}} <span v-if="obj.etype === 1">{{obj.buyCurrency}}</span><span
              v-if="obj.etype === 2">{{obj.sellCurrency}}</span></i></p>
        </div>
        <table class="pcTableDetial" v-if="true">
          ...
          <tr>
            ...
            <td>{{obj.price .toFixed(8)}} USD</td>
          </tr>
          <tr>
            <td>{{$t('sieOrderDetail.text30')}}</td>
            <td>{{obj.amount .toFixed(8)}} {{obj.currency}}</td>

          <tr>
            <td>{{$t('exchanges.text26')}}</td>
            <td>{{obj.tradeFee .toFixed(8)}} USD</td>
          </tr>
          <tr>
            <td>{{$t('sieOrderDetail.text31')}}</td>
            <td>{{obj.cashAmount .toFixed(8)}} USD</td>
          </tr>
          <tr>
            <td>{{$t('sieOrderDetail.text32')}}</td>
            <td>{{obj.tradeAmount .toFixed(8)}} {{obj.currency}}</td>
          </tr>
          <tr>
            <td>{{$t('sieOrderDetail.text33')}}</td>
            <td>{{obj.tradeCashAmount .toFixed(8)}} USD</td>
          </tr>
          <tr v-if="obj.actNo === ''">
        ...
        <table class="commonTableList">
          ...
          <tr v-for="(item,index) in orderDetailList" :key="index">
            <td>{{item.tradePrice .toFixed(8)}} USD</td>
            <td>{{item.tradeAmount .toFixed(8)}} {{obj.currency}}</td>
            <td>{{$changeDate(item.createAt, '/')}}</td>
            <td class="time">{{item.tradeTotalPrice .toFixed(8)}} USD</td>
          </tr>
          </tbody>
          <!--<div v-if="orderDetailList.length === 0 || Number(obj.state) === 4 || Number(obj.state) === 5" class="error">{{$t('orderManage.text8')}}</div>-->
          <v-turnPage class="turnPage" v-if="orderDetailList.length !== 0" :propsPage="entrustTotal"
                      @sieTradeTrun="rechargeTurnPage"></v-turnPage>
        </table>
    ...
    <div class="mobile">
      <div class="management-head">
        <div class="head">
          ...
          <div class="title">{{$t('exchanges.text20')}}</div>
          <div class="right"></div>
        </div>
      </div>
      <div class="mobile-content">
        <div class="order-progress">
          ...
          <p><span>{{obj.tradeAmount .toFixed(8)}} <span v-if="obj.etype === 1">{{obj.buyCurrency}}</span><span
            v-if="obj.etype === 2">{{obj.sellCurrency}}</span>  </span> / <i>{{obj.amount .toFixed(8)}} <span
            v-if="obj.etype === 1">{{obj.buyCurrency}}</span><span
            v-if="obj.etype === 2">{{obj.sellCurrency}}</span></i></p>
        </div>
        ...
        <div class="mobile-box">
          <ul v-if="true">
            ...
            <li>
              <span v-if="obj.etype === 1">{{$t('sieOrderDetail.text10')}}:</span><span v-if="obj.etype === 2">{{$t('sieOrderDetail.text11')}}:</span><i>{{obj.price
              .toFixed(8)}} USD</i>
            </li>
            <li>
              <span>{{$t('sieOrderDetail.text30')}}:</span><i>{{obj.amount .toFixed(8)}} {{obj.currency}}</i>
            </li>
            <li>
              <span>{{$t('sieOrderDetail.text31')}}:</span><i>{{obj.cashAmount .toFixed(8)}} USD</i>
            </li>
            <li>
              <span>{{$t('exchanges.text26')}}:</span><i>{{obj.tradeFee .toFixed(8)}} USD</i>
            </li>
            <li>
              <span>{{$t('sieOrderDetail.text32')}}:</span><i>{{obj.tradeAmount .toFixed(8)}} {{obj.currency}}</i>
            </li>
            <li>
              <span>{{$t('sieOrderDetail.text33')}}:</span><i>{{obj.tradeCashAmount .toFixed(8)}} USD</i>
            </li>
            <li v-if="(obj.etype !== 1)">
  ...
  export default {
    ...
    methods: {
      ...
      confirmPay() {
        ...
        this.$http.post(`${this.$api}/newCoinentrustRecord/cancelRecord`, fmData, {headers: {'secret-language': ajaxLang1}}).then(res => {
          if (res.data.success) {
      ...
      toDetail() {
        this.$router.push({
          path: '/account/exchanges/mobileDetail',
          query: {
